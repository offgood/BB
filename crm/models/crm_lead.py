...
class Lead(models.Model):
    ...
    user_id = fields.Many2one('res.users', string='ผู้ขายงาน', index=True, tracking=True, default=lambda self: self.env.user)
    ...
    priority = fields.Selection(
        crm_stage.AVAILABLE_PRIORITIES, string='ความสำคัญ', index=True,
    ...
    tag_ids = fields.Many2many(
        'crm.tag', 'crm_tag_rel', 'lead_id', 'tag_id', required=True, string='ประเภทงาน',
    ...
    project_id = fields.Many2one("project.project", string='โปรเจค',tracking=True)
    # Dates
    ...
    contact_name = fields.Char(
        'ลูกค้า', tracking=30,
        compute='_compute_contact_name',required=True, readonly=False, store=True)
    partner_name = fields.Char(
        'บริษัท/หน่วยงาน', index=True,
        compute='_compute_partner_name', tracking=True,readonly=False, required=True, store=True,
        help='The name of the future partner company that will be created while converting the lead into opportunity')
    function = fields.Char('ตำแหน่ง', tracking=True, required=True, compute='_compute_function', readonly=False, store=True)
    title = fields.Many2one('res.partner.title', tracking=True,string='คำนำหน้า', compute='_compute_title', required=True, readonly=False, store=True)
    email_from = fields.Char(
        'Email', tracking=True, index=True,
        compute='_compute_email_from', inverse='_inverse_email_from', required=True, readonly=False, store=True)
    fax = fields.Char(string='แฟกซ์', tracking=True)
    phone = fields.Char(
        'เบอร์โทรศัพท์', tracking=True,required=True,
        compute='_compute_phone', inverse='_inverse_phone', readonly=False, store=True)
    mobile = fields.Char('เบอร์มือถือ', tracking=True,compute='_compute_mobile', readonly=False, store=True)
    ...
    source_leads = fields.Many2one('crm.lead.sourceleads','ที่มาของ งาน', tracking=True,required=True)
    ...
    website = fields.Char('เว็บไซต์', index=True, help="Website of the contact", compute="_compute_website", readonly=False, store=True)
    lang_id = fields.Many2one('res.lang', string='Language')
    # Address fields
    tree = fields.Many2one('res.city.zip','เลือกที่อยู่', required=True,tracking=True)
    street = fields.Char('ที่อยู่',related='tree.display_name', compute='_compute_partner_address_values', readonly=False, store=True)
    street2 = fields.Char('แขวง', compute='_compute_partner_address_values', readonly=False, store=True)
    ...
    zip = fields.Char('รหัสไปรษณี',related='tree.name', tracking=True,change_default=True, compute='_compute_partner_address_values', readonly=False, store=True)
    city = fields.Char('เขต', compute='_compute_partner_address_values', readonly=False, store=True)
    state_id = fields.Many2one(
        "res.country.state", string='จังหวัด', related='tree.state_id',
        compute='_compute_partner_address_values', tracking=True,readonly=False, store=True,
        domain="[('country_id', '=?', country_id)]")
   
    country_id = fields.Many2one(
        'res.country', string='ประเทศ',
        compute='_compute_partner_address_values',related='tree.country_id', readonly=False, store=True)
    Note_address = fields.Text('ที่อยู่เพิ่มเติม',compute="_compute_sub_string_name_res_city",readonly=False, store=True)
    # Address invoices
    ...
    Note_address_invoice = fields.Text('ที่อยู่เพิ่มเติม',compute="_compute_sub_string_name_res_city_invoice", store=True, readonly=False)
     # Address delivery
    ...
    Note_address_delivery = fields.Text('ที่อยู่เพิ่มเติม', compute="_compute_sub_string_name_res_city_delivery", store=True,readonly=False)
    #เอามาจาก opp 
    project_id = fields.Many2one("project.project", string='Project', tracking=True)
    # Probability (Opportunity only)
    probability = fields.Float(
        'อัตราความสำเร็จ', group_operator="avg", copy=False,
    ...
    @api.model
    def get_project_id(self, lead_opp):
        _logger.info(lead_opp.lead_id)
        ref = lead_opp.lead_id.write({
            'project_id' : lead_opp.project_id.id
        })
        _logger.info(ref)
        return ref
                 
    @api.onchange('tree')#เอาไว้ substr
    def _compute_sub_string_name_res_city(self):
        ...
        for rec in self:
            if rec.tree:
                address = rec.tree.display_name
                str__district = address.split(',')
                if rec.tree.details:#มีdetailsให้ลง Note
    ...
    def _compute_sub_string_name_res_city_invoice(self):
        ...
        for rec in self:
            if rec.street_invoice:
                ...
                str__district = address.split(',')
                if rec.street_invoice.details:#มีdetailsให้ลง Note
    ...
    def _compute_sub_string_name_res_city_delivery(self):
        ...
        for rec in self:
            if rec.street_delivery:
                ...
                str__district = address.split(',')
                if rec.street_delivery.details:#มีdetailsให้ลง Note
...
class Organizadetails(models.Model):
    _name = 'crm.lead.sourceleads'
    _description = 'Source Leads'
    _rec_name = 'name'
